...
class StringsTask2(Task):
    ...
    def _generate_condition_task(self, seed: int) -> str:
        ...
        if not os.path.exists(task_path):
            raise FileNotFoundError(
                'Описание задания для strings_2 не найдено.'
            )
    ...
    def _generate_code_template(self, seed: int) -> str:
        base_path = os.path.dirname(__file__)
        task_path = os.path.join(base_path, 'templates',
                                 'strings_2_template_code.txt')

        if not os.path.exists(task_path):
            raise FileNotFoundError('Код задания для strings_2 не найдено.')
    ...
    def _generate_template_coderunner(self, seed: int) -> str:
        ...
        if not os.path.exists(task_path):
            raise FileNotFoundError(
                'Coderunner шаблон для strings_2 не найден.'
            )
