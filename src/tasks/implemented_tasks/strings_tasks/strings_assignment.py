...
TEMPLATE_PATH = (TEMPLATES_DIR / 'string'
                 / 'strings_assignment_template.toml')
...
class StringsAssignment(ABCTask):

    name = 'strings_assignment_task'
    ...
    def __generate_param(self) -> dict[str, str]:
        ...
        params = {
            # Test 3: assignment
            "string_3": ValueGenerator.generate_string_from_struct(
                self._config['strings_1']
            ),
            "index_3_1": ValueGenerator.generate_index_from_struct(
                self._config['strings_1']
            ),
            "index_3_2": ValueGenerator.generate_index_from_struct(
                self._config['strings_1']
            ),
            "index_3_3": ValueGenerator.generate_index_from_struct(
                self._config['strings_1']
            ),
            "index_3_4": ValueGenerator.generate_index_from_struct(
                self._config['strings_1']
            ),
            "char_3_1": ValueGenerator.generate_char(),
            "char_3_2": ValueGenerator.generate_char(),
            "char_3_3": ValueGenerator.generate_char(),
        }
